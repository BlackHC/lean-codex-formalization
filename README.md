...
- Add `mathlib` as a dependency in `lakefile.lean` and fetch its import via `lake update`. This provides probability theory, combinatorics, and finitary graph structures required throughout the development.
- Establish a project-wide namespace (e.g., `Codex`) and set up linting/CI commands (`lake build`, `lake exe cache get` if needed).
...
1. **Finite simple graphs.**
   - Use `SimpleGraph` from `mathlib` with vertex type `Fin n`. Define helper constructors for labelled graphs on `Fin n` with explicit edge sets.
   - Verify basic facts about edge counts (`e(H)`) and subgraph inclusion with Lean lemmas, tagging the most important ones with `@[simp]` or `@[simp, aesop_safe]` for later automation.
   - Create test lemmas (e.g., `example : e (completeGraph (Fin 3)) = 3 := by decide`) to confirm the definitions match combinatorial intuition.

2. **Copy counting (`M_{H', H}`).**
   - Define `countCopies (H' H : SimpleGraph (Fin n)) : ℕ` as the number of embeddings of `H'` into `H` (using `SimpleGraph.embedding`), quotienting by automorphisms if needed.
   - Prove double-counting identities, e.g., the equality `π_H(J₀ ⊆ 𝐇) = M_{J,H} / M_J` via Lean proofs using `Fintype.card` and symmetry.
   - Validate the combinatorial identities with small examples (`Fin 3`, `Fin 4`) inside Lean using `dec_trivial` or `simp [countCopies]` to ensure the formulas have the correct normalization factors.
...
1. Update `lakefile.lean` to import `mathlib` and verify a trivial build (`lake build`).
2. Start Stage 1 by formalizing graph embeddings and copy counting.
3. Incrementally translate each lemma from the paper, aligning Lean filenames with paper sections (e.g., `Formalization/SpreadLemma.lean`).
